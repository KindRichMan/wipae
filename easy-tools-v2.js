...
const LAYOUT_KEY='wipae.easy.layout.v5';
const VERSION=5;
const $=s=>document.querySelector(s);
const DEFAULT_LAYOUT={fontScale:100,spacing:0,colGap:0,x:0,y:0,colR:0,colL:0};
const X_LIMIT=12, Y_LIMIT=14;               // 위패 전체 위치 이동 한계(%)
const COL_LIMIT=45;                          // 줄(열) 하나의 좌우 이동 한계(자기 폭 기준 %)
...
function currentLayout(){
  ...
  return {
    ...
    colR:+($('#easyColR')?.value||DEFAULT_LAYOUT.colR),
    colL:+($('#easyColL')?.value||DEFAULT_LAYOUT.colL),
...
function updateOutputs(){
  [['easyFontSize','easyFontSizeOut'],['easySpacing','easySpacingOut'],['easyColGap','easyColGapOut'],['easyMoveX','easyMoveXOut'],['easyMoveY','easyMoveYOut'],['easyColR','easyColROut'],['easyColL','easyColLOut']]
...
function applyVisual(save=true){
  ...
  root.setProperty('--easy-col-r',clamp(l.colR,COL_LIMIT)+'%');
  root.setProperty('--easy-col-l',clamp(l.colL,COL_LIMIT)+'%');
...
function applyLayout(l){
  ...
  set('easyColR',clamp(l.colR,COL_LIMIT));set('easyColL',clamp(l.colL,COL_LIMIT));
  const n=nativeSize();
...
function resetLayout(){
  [['easyFontSize',100],['easySpacing',0],['easyColGap',0],['easyMoveX',0],['easyMoveY',0],['easyColR',0],['easyColL',0]]
...
function inject(){
  ...
  :root{--easy-spacing:0px;--easy-colgap:0px;--easy-x:0%;--easy-y:0%;--easy-scale:1;--easy-col-r:0%;--easy-col-l:0%}
  ...
  .zone>.col:first-child{transform:translateX(var(--easy-col-r))!important}
  .zone>.col:last-child{transform:translateX(var(--easy-col-l))!important}
  ...
  const p=document.createElement('aside');p.id='easyToolsPanel';p.innerHTML=`
    ...
    <div class="easy-help" style="margin:4px 0 -2px">아래는 두 줄(오른쪽·왼쪽)을 각각 좌우로 움직입니다. − 왼쪽 / + 오른쪽</div>
    <div class="easy-slider"><span>오른쪽 줄</span><input id="easyColR" type="range" min="-45" max="45" value="0"><output id="easyColROut">0</output></div>
    <div class="easy-slider"><span>왼쪽 줄</span><input id="easyColL" type="range" min="-45" max="45" value="0"><output id="easyColLOut">0</output></div>
  ...
  ['easyFontSize','easySpacing','easyColGap','easyMoveX','easyMoveY','easyColR','easyColL'].forEach(id=>$('#'+id).oninput=()=>applyVisual());
